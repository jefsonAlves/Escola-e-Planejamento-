...
import { School, AttendanceRecord, GradeRecord } from "../../types";
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, LineChart, Line, CartesianGrid, Legend } from "recharts";
import { Shield, TrendingUp, Users, BookOpen, Calendar as CalendarIcon, Filter } from "lucide-react";
...
export const SchoolDashboardScreen: React.FC<Props> = ({
  ...
  const [attendanceData, setAttendanceData] = useState<{ date: string; presence: number; absence: number }[]>([]);
  const [gradesData, setGradesData] = useState<{ subject: string; average: number }[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch Attendance
        // Use all attendance without date filter if index is not ready, or filter in memory
        const attendanceSnap = await getDocs(query(collection(db, `schools/${school.id}/attendance`)));
        ...
        attendanceSnap.forEach((doc) => {
          ...
          if (data.date >= dateStrThreshold) {
             let key = "";
             if (period === "monthly") {
                 key = new Date(data.date).toLocaleDateString("pt-BR", { day: "2-digit", month: "short" });
             } else {
                 key = new Date(data.date).toLocaleDateString("pt-BR", { month: "short", year: "numeric" });
             }

            if (!attendanceMap[key]) attendanceMap[key] = { presence: 0, absence: 0 };
            if (data.status === "present") attendanceMap[key].presence++;
            else attendanceMap[key].absence++;
          }
        });

        // Depending on period, sort days or months correctly
        // Here we just accept simple string sorting or timestamp base sorting
        const aData = Object.keys(attendanceMap).map(key => ({
            date: key,
        ...
        // Fetch Grades
        const gradesSnap = await getDocs(query(collection(db, `schools/${school.id}/grades`)));
        const subjectMap: Record<string, { total: number; count: number }> = {};
        
        gradesSnap.forEach((doc) => {
          const data = doc.data() as GradeRecord;
          
          // Optionally filter grades based on Date if the record had it. For now, we filter broadly or not at all since grades might not have a date
          if (!subjectMap[data.subject]) subjectMap[data.subject] = { total: 0, count: 0 };
          subjectMap[data.subject].total += data.value;
          subjectMap[data.subject].count++;
        });

        const gData = Object.keys(subjectMap).map(subject => ({
          subject,
          average: Number((subjectMap[subject].total / subjectMap[subject].count).toFixed(1))
        }));
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-5 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col justify-center">
            ...
            <p className="text-2xl font-black text-slate-800 dark:text-slate-100">{totalStudents}</p>
            <p className="text-xs font-bold text-slate-500 uppercase mt-1">Alunos</p>
        </div>
        <div className="p-5 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col justify-center">
            ...
            <p className="text-2xl font-black text-slate-800 dark:text-slate-100">{totalTeachers}</p>
            <p className="text-xs font-bold text-slate-500 uppercase mt-1">Professores</p>
        </div>
        <div className="p-5 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col justify-center">
            ...
            <p className="text-2xl font-black text-slate-800 dark:text-slate-100">{totalClasses}</p>
            <p className="text-xs font-bold text-slate-500 uppercase mt-1">Turmas</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={attendanceData}>
                  ...
                  <Line type="monotone" name="Presenças" dataKey="presence" stroke="#10b981" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                  <Line type="monotone" name="Faltas" dataKey="absence" stroke="#f43f5e" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm">
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2">
            ...
            Média de Notas por Disciplina ({period === "monthly" ? "Mensal" : "Anual"})
          </h3>
          <div className="h-64">
            {loading ? (
               <div className="h-full flex items-center justify-center text-slate-400">Carregando gráfico...</div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                 <BarChart data={gradesData} layout="vertical" margin={{ top: 0, right: 30, left: 20, bottom: 0 }}>
                    ...
                    <Tooltip 
                      ...
                      contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}
                    />
                    <Bar dataKey="average" name="Média" fill="#3b82f6" radius={[0, 4, 4, 0]} barSize={24} />
                 </BarChart>
              </ResponsiveContainer>
            ) : (
                <div className="h-full flex flex-col items-center justify-center text-slate-400">
                   <BookOpen className="w-8 h-8 mb-2 opacity-20" />
                   <span className="text-sm font-medium">Nenhuma nota registrada</span>
                   <span className="text-xs">Registre avaliações para visualizar</span>
                </div>
            )}
          </div>
        </div>
